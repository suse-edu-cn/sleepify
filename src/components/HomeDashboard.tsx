...
export default function HomeDashboard() {
  ...
  const sleepContent = useMemo(() => {
    ...
    if (!sleepStatus) {
      return <div className="sleepy-card-sleep-status">
    ...
    if (sleepStatus.status === 0) {
      return (
        <>
          <div className="sleepy-card-sleep-status">
            <mdui-icon-bedtime />
            当前无进行中的睡眠活动
          </div>
          <mdui-button
    ...
    const hasValidRange = Number.isFinite(startTime) && Number.isFinite(endTime) && endTime > startTime;
    ...
    const progressPercent = hasValidRange
      ? Math.max(0, Math.min(100, Math.round(((nowMs - startTime) / (endTime - startTime)) * 100)))
      : 0;
    ...
    return (
      <>
        ...
        {hasValidRange ? (
          <>
            <div className="sleepy-sleep-progress">
              <div>已开始：{getTimeStr(elapsedMinutes)}</div>
              <div>还剩余：{getTimeStr(remainingMinutes)}</div>
            </div>
            <mdui-linear-progress max={100} value={progressPercent}></mdui-linear-progress>
            <div className="sleepy-sleep-progress-percent">{progressPercent}%</div>
          </>
        ) : null}
        <div className="sleepy-time-grid">
          <div className="sleepy-time-tag">
  ...
  return (
    <section className="sleepy-page">
      <mdui-card className="sleepy-card">
        <div className="sleepy-card-title-row">
          <h2 className="sleepy-card-title">睡眠状态</h2>
          <button
            type="button"
            className="sleepy-refresh-btn"
            aria-label="刷新睡眠状态"
      ...
      <mdui-card className="sleepy-card">
        <div className="sleepy-card-title-row">
          <h2 className="sleepy-card-title">积分</h2>
          <button
            type="button"
            className="sleepy-refresh-btn"
            aria-label="刷新积分"
        ...
        {userLoading ? (
          ...
          <div className="sleepy-card-body">
